function day2() {
    ...
    function phase2(spreadsheet) {
        ...
        lines.forEach(line => {
            var digits = line.split('\t');
            var found = false;
            for (let i = 0; i < digits.length && !found; i++) {
                for (let j = 0; j < digits.length && !found; j++) {
                    if (i !== j && digits[i] % digits[j] == 0) {
                        var d = Math.floor(digits[i] / digits[j]);
                        found = true;
                    }
                }
            }
...
console.log(day2().P2(t));
var token = "1919	2959	82	507	3219	239	3494	1440	3107	259	3544	683	207	562	276	2963\n \
